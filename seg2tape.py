...
class Application(Frame):
    def __init__(self,root):
        ...
        self.prompt.title("Welcome! please login.")
        Label(self.prompt,text='username:').grid(row=0,column=0)
        Label(self.prompt,text='password:').grid(row=1,column=0)
        self.username=Entry(self.prompt)
        self.password=Entry(self.prompt)
        self.username.grid(row=0,column=1)
        self.password.grid(row=1,column=1)
        Button(self.prompt,text='Authenticate user',command=self.authenticate).grid(row=2,column=1)
        self.prompt.mainloop()


    def authenticate(self):
        user_dn = "uid="+self.username.get()+",ou=student,dc=domain,dc=com"
        # adjust this to your base dn for searching
        base_dn = "dc=domain,dc=com"
        connect = ldap.initialize('ldap://localhost')
        search_filter = "uid="+self.username.get()
        try:
        #if authentication successful, get the full user data
            connect.bind_s(user_dn,self.password.get())
    ...
    def create_widgets(self):
        ...
        self.e=Entry(self,state='normal',highlightthickness=3,highlightbackground='black')
    ...
    def close_app(self):
        if messagebox.askokcancel("Close","Are you sure?"):
            print("the parent is "+str(os.getppid()))
            #os.system('chmod u+x check_status.sh')
